<div align="center">

<!-- HEADER BANNER -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0a0a0f,50:0e4f5c,100:22d3ee&height=200&section=header&text=David%20Estevez&fontSize=50&fontColor=ffffff&fontAlignY=35&desc=CTO%20·%20Software%20Architect%20·%20Mathematician&descSize=18&descAlignY=55&descColor=a1a1aa&animation=fadeIn" width="100%" />

<!-- TYPING ANIMATION -->
<a href="https://destbreso.com">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&pause=1000&color=22D3EE&center=true&vCenter=true&multiline=true&repeat=true&width=600&height=60&lines=Building+zero-to-scale+platforms+with+math+%26+code;CTO+%40+Coverfleet+%C2%B7+10%2B+years+in+engineering;Architecture+%C2%B7+AI+%C2%B7+Distributed+Systems" alt="Typing SVG" />
</a>

<br/>

<!-- PROFILE VIEWS -->
<img src="https://komarev.com/ghpvc/?username=destbreso&style=for-the-badge&color=0e4f5c&label=PROFILE+VIEWS" />

<br/><br/>

<!-- SOCIAL BADGES -->
[![Website](https://img.shields.io/badge/🌐_destbreso.com-22d3ee?style=for-the-badge&logoColor=white)](https://destbreso.com)
[![Blog](https://img.shields.io/badge/📝_Blog-0e4f5c?style=for-the-badge)](https://destbreso.com/blog)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/destbreso)
[![Twitter](https://img.shields.io/badge/𝕏-000000?style=for-the-badge&logo=x&logoColor=white)](https://twitter.com/destbreso)

</div>

---

### 👋 About Me

I'm a **Mathematician turned CTO** — I started in pattern recognition research at CENATAV's biometrics lab, co-founded startups, and now lead technology strategy at **[Coverfleet](https://coverfleet.com)**. With 10+ years building platforms from zero to scale, I'm obsessed with clean architecture, first-principles thinking, and engineering tools that make teams move faster.

> *"The same rigor you apply to a topology proof can design a bulletproof distributed system."*

📍 Miami, FL &nbsp;·&nbsp; 🎓 B.Sc. Mathematics, Universidad de La Habana &nbsp;·&nbsp; 🗣 English & Spanish

---

### 🛠 Tech Stack

<div align="center">

**Backend & Runtime**

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=flat-square&logo=nestjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)

**Frontend**

![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)

**Data & Infrastructure**

![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonwebservices&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)

**Architecture & AI**

![Microservices](https://img.shields.io/badge/Microservices-FF6F00?style=flat-square&logo=fastapi&logoColor=white)
![DDD](https://img.shields.io/badge/DDD-8B5CF6?style=flat-square)
![Event Driven](https://img.shields.io/badge/Event_Driven-22C55E?style=flat-square)
![LLM Agents](https://img.shields.io/badge/LLM_Agents-F59E0B?style=flat-square&logo=openai&logoColor=white)

</div>

---

### 🚀 Featured Projects

<table>
<tr>
<td width="50%">

**🚛 [Coverfleet](https://coverfleet.com)** `Professional`
<br/>Full-stack fleet management platform with AI-powered DVIR inspections, real-time ELD sync, and compliance reporting. Built from zero to production as CTO.
<br/>`Next.js` `NestJS` `MongoDB` `TypeScript` `BullMQ` `AWS`

</td>
<td width="50%">

**🤖 [Agentic Lab](https://github.com/destbreso/agentic-lab)**
<br/>Extensible platform for building autonomous agentic loops and AI workflows. Multi-provider support, tool orchestration, and real-time dashboard.
<br/>`TypeScript` `Node.js` `PostgreSQL` `Redis` `Docker`

</td>
</tr>
<tr>
<td width="50%">

**📄 [CV Generator](https://github.com/destbreso/cv-generator)**
<br/>AI-powered resume builder with 18+ professional templates, multi-LLM enhancement, real-time preview, and PDF export.
<br/>`Next.js` `React 19` `Tailwind v4` `Cloudflare Workers`

</td>
<td width="50%">

**🔥 [Load Testing Lab](https://github.com/destbreso/load-testing-lab)**
<br/>Complete load testing platform with professional CLI, k6 + Artillery runners, InfluxDB metrics, and Grafana dashboards.
<br/>`k6` `Artillery` `InfluxDB` `Grafana` `Docker`

</td>
</tr>
<tr>
<td width="50%">

**⚡ [LLM Worker Lab](https://github.com/destbreso/llm-worker-lab)**
<br/>Scalable async LLM request processing with smart rate limiting, priority queues, worker pools, and Prometheus observability.
<br/>`Node.js` `Redis` `Prometheus` `Docker`

</td>
<td width="50%">

**📊 [Queue System Simulation](https://github.com/destbreso/queue-system-simulation)**
<br/>Interactive visualization of queueing theory models with real-time parameter tuning and animated queue behavior.
<br/>`Next.js` `React` `TypeScript` `shadcn/ui`

</td>
</tr>
</table>

---

### ✍️ Latest Writing

<!-- BLOG POSTS -->
|    | Article                                                                                                          | Where         |
|----|------------------------------------------------------------------------------------------------------------------|---------------|
| 🔥 | **[Mastering API Load Testing — From Basics to Chaos Engineering](https://destbreso.com/blog)**                  | destbreso.com |
| 🚀 | **[Building a Production Load Testing Lab: A Developer's Journey](https://destbreso.com/blog)**                  | destbreso.com |
| 📐 | **[Strategic Approaches to Performance Engineering](https://destbreso.com/blog)**                                | destbreso.com |
| ⚡ | **[Advanced Traffic Patterns and Chaos Testing](https://destbreso.com/blog)**                                    | destbreso.com |
| 🔧 | **[Integrating Load Tests with CI/CD Pipelines](https://destbreso.com/blog)**                                   | destbreso.com |

📚 **[Read all articles on destbreso.com →](https://destbreso.com/blog)**

---

### 📊 GitHub Stats

<div align="center">

<!-- TROPHY SHELF -->
<img src="https://github-profile-trophy.vercel.app/?username=destbreso&theme=darkhub&no-bg=true&no-frame=true&column=7&margin-w=12" width="100%" />

<br/>

<img src="https://github-readme-stats.vercel.app/api?username=destbreso&show_icons=true&theme=github_dark&hide_border=true&bg_color=0a0a0f&title_color=22d3ee&icon_color=22d3ee&text_color=a1a1aa&ring_color=22d3ee&include_all_commits=true&count_private=true" height="170" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=destbreso&layout=compact&theme=github_dark&hide_border=true&bg_color=0a0a0f&title_color=22d3ee&text_color=a1a1aa&langs_count=8" height="170" />

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com?user=destbreso&theme=github-dark-blue&hide_border=true&background=0A0A0F&ring=22D3EE&fire=22D3EE&currStreakLabel=22D3EE&sideLabels=A1A1AA&dates=71717A" />

<br/>

<!-- CONTRIBUTION GRAPH -->
<img src="https://github-readme-activity-graph.vercel.app/graph?username=destbreso&theme=github-dark&hide_border=true&bg_color=0a0a0f&color=22d3ee&line=22d3ee&point=ffffff&area=true&area_color=22d3ee" width="100%" />

</div>

---

### 🧠 Deep Insights

<div align="center">

<!-- PROFILE SUMMARY CARDS — unique visualizations -->
<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=destbreso&theme=github_dark" width="100%" />

<img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=destbreso&theme=github_dark" height="170" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=destbreso&theme=github_dark" height="170" />

<img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=destbreso&theme=github_dark" height="170" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=destbreso&theme=github_dark&utcOffset=-4" height="170" />

<!-- CONTRIBUTION SNAKE -->
<br/>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/destbreso/destbreso/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/destbreso/destbreso/output/github-snake.svg" />
  <img alt="github contribution snake animation" src="https://raw.githubusercontent.com/destbreso/destbreso/output/github-snake-dark.svg" width="100%" />
</picture>

</div>

---

### 📌 Pinned Repositories

<div align="center">

<a href="https://github.com/destbreso/agentic-lab">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=destbreso&repo=agentic-lab&theme=github_dark&hide_border=true&bg_color=0a0a0f&title_color=22d3ee&icon_color=22d3ee&text_color=a1a1aa" />
</a>
<a href="https://github.com/destbreso/load-testing-lab">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=destbreso&repo=load-testing-lab&theme=github_dark&hide_border=true&bg_color=0a0a0f&title_color=22d3ee&icon_color=22d3ee&text_color=a1a1aa" />
</a>
<a href="https://github.com/destbreso/cv-generator">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=destbreso&repo=cv-generator&theme=github_dark&hide_border=true&bg_color=0a0a0f&title_color=22d3ee&icon_color=22d3ee&text_color=a1a1aa" />
</a>
<a href="https://github.com/destbreso/llm-worker-lab">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=destbreso&repo=llm-worker-lab&theme=github_dark&hide_border=true&bg_color=0a0a0f&title_color=22d3ee&icon_color=22d3ee&text_color=a1a1aa" />
</a>

</div>

---

<div align="center">

**🌐 [destbreso.com](https://destbreso.com)** — Full portfolio, blog & more

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0a0a0f,50:0e4f5c,100:22d3ee&height=100&section=footer" width="100%" />

</div>
